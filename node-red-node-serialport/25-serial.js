...
module.exports = function(RED) {
    ...
    function SerialPortNode(n) {
        ...
        this.pidvidserial = n.pidvidserial;
    }
        ...
        return {
            get:function(serialConfig) {
                ...
                var port = serialConfig.serialport,
                    pidvidserial = serialConfig.pidvidserial, // ADDED BY ME
                ...
                connections[id] = (function() {
                    ...
                    var olderr = "";
                    var setupSerial = async function() {
                        // I THINK THIS IS WHERE I NEED TO SEARCH FOR PORT
                        var ports_list = [];

                        // var ports_list;
                        // ports_list = SerialPort.list()
                        ...
                        var pid_vid_serial = pidvidserial.split(":");
                        var serial_num = pid_vid_serial[2];
                        var pid = pid_vid_serial[1];
                        var vid = pid_vid_serial[0];
                        // RED.log.info("[ports list: ["+ports_list[0]['path']+"] "+RED._("serial.onopen",{port:port,baud:baud,config: databits+""+parity.charAt(0).toUpperCase()+stopbits}));
                        // port = pid_vid_serial
                        for (let i = 0; i < ports_list.length; i++) {
                            
                            if ((pid == ports_list[i]['productId']) && (vid == ports_list[i]['vendorId']) && (serial_num == ports_list[i]['serialNumber'])) {
                                port = ports_list[i]['path'];
                                break;
                            }                            
                        }
                        // serialConfig.serialport = "CHECK"
                        
                          
                        // THIS IS THE NEW STUFF I ADDED, it probably WONT WORK
